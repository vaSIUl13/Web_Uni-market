...
const Header = () => {
  ...
  // 1. Додаємо функцію для плавного скролу
  const handleScroll = (e: React.MouseEvent<HTMLAnchorElement>, id: string) => {
    e.preventDefault(); // Зупиняємо стандартний різкий стрибок браузера
    
    const element = document.getElementById(id);
    if (element) {
      // Рахуємо позицію: відступ елемента від верху мінус висота шапки (десь 80px)
      const y = element.getBoundingClientRect().top + window.scrollY - 80; 
      
      window.scrollTo({ top: y, behavior: 'smooth' });
    }
    
    // Якщо клікнули з мобільного меню — закриваємо його
  ...
  return (
    <header className="w-full bg-white border-b border-gray-100 sticky top-0 z-50">
      <div className="max-w-[1280px] mx-auto px-4 py-4 flex items-center justify-between">
        ...
        {/* 2. Навігація по центру (Desktop) - Оновлено onClick */}
        <nav className="hidden lg:flex items-center gap-2">
          <a href="#" className="bg-[#eff6ff] text-[#3b63f6] px-5 py-2.5 rounded-full text-sm font-semibold transition-colors">Головна</a>
          <a href="#" className="text-gray-600 hover:bg-gray-50 hover:text-[#3b63f6] px-5 py-2.5 rounded-full text-sm font-medium transition-colors">Каталог</a>
          <a 
            href="#categories" 
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          <button className="flex items-center gap-2 border border-[#3b63f6] text-[#3b63f6] px-6 py-2.5 rounded-full text-sm font-semibold hover:bg-[#eff6ff] transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" /></svg>
            Увійти
          </button>
        ...
        >
          {isMobileMenuOpen ? (
            <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
          ) : (
            <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h16" /></svg>
          )}
        </button>
      </div>

      {/* 3. Мобільне меню - Оновлено onClick */}
      <div className={`lg:hidden fixed inset-0 bg-white z-40 flex flex-col pt-24 px-6 transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <nav className="flex flex-col gap-4 text-center text-lg font-medium">
          <a href="#" className="bg-[#eff6ff] text-[#3b63f6] py-3 rounded-2xl">Головна</a>
          <a href="#" className="py-3 text-gray-700 hover:text-[#3b63f6]">Каталог</a>
          <a 
            href="#categories" 
            onClick={(e) => handleScroll(e, 'categories')} 
        ...
        <div className="mt-auto mb-10 flex flex-col gap-3">
          <button className="w-full flex justify-center items-center gap-2 border border-[#3b63f6] text-[#3b63f6] py-3.5 rounded-2xl font-semibold">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" /></svg>
            Увійти
          </button>
