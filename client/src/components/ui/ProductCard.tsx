...
const ProductCard = ({ imageUrl, topBadge, category, title, seller, rating, reviewsCount }: ProductCardProps) => {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300 group cursor-pointer flex flex-col h-full">
      ...
      <div className="relative h-56 w-full overflow-hidden">
        ...
        {topBadge && (
          <div className={`absolute top-3 left-3 ${topBadge.bgClass} text-white text-xs font-bold px-3 py-1 rounded-lg shadow-sm`}>
            {topBadge.text}
          </div>
        )}
      ...
      <div className="p-5 flex flex-col flex-grow">
        <div className="mb-3">
          <span className={`inline-block px-2.5 py-1 rounded-md text-[11px] font-bold ${category.bgClass} ${category.textClass}`}>
            {category.text}
          </span>
        </div>
        ...
        <div className="mt-auto">
          <p className="text-xs text-gray-500 mb-2">
            Продавець: <span className="font-medium text-gray-700">{seller}</span>
          </p>
